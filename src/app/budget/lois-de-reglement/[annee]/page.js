...
export async function generateMetadata({ params }) {
  ...
  return { title: `Loi de règlement ${annee} — Ministère des Finances du Niger` };
}

export default async function Page({ params }) {
  const { annee } = await params;
  const data = await getBudgetDocuments('lois-de-reglement', annee);
  ...
  return (
    <>
      <PageHero
        surtitre="Budget"
        titre={`Loi de règlement — gestion ${annee}`}
        sousTitre="Texte de loi de règlement de l'année budgétaire. Cliquez pour consulter ou télécharger."
        fil={[
          { label: 'Accueil', href: '/' },
          ...
          {documents.length === 0 ? (
            <Box sx={{ backgroundColor: '#fff', border: `1px solid ${C.border}`, borderRadius: 3, p: { xs: 3, md: 5 }, textAlign: 'center' }}>
              <Typography sx={{ fontWeight: 800, color: C.blue, fontSize: '1.2rem', mb: 1 }}>
                Document en cours de mise en ligne
              </Typography>
              <Typography sx={{ color: C.muted }}>
                La loi de règlement de la gestion {annee} sera publiée prochainement.
              </Typography>
            </Box>
          ) : (
            <>
